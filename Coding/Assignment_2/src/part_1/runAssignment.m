...
for d = 1:3
    ...
    for continuity = 0:d-1
        ...
        fOut = fopen(fName,'w+');
        fprintf(fOut,"%s\n","solutionType, Error, NumElements, ElementDegree, Continuity, f, g, h");
        for n = 1:length(nELEM)
            ...
            fprintf(fOut,"%s,%.16E,%d,%d,%s,%s,%s,%s\n",solutionType,Error,NumElements,ElementDegree,num2str(continuity),string(f),string(g),string(h));
        end
        fclose('all');
    end
end
