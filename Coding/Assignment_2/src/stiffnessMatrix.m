function [K,k] = stiffnessMatrix(ELEM,eCONN,b,C,method)
nELEM = length(ELEM);
...
if method == "Exact"
    ...
    for e = 1:nELEM
        ...
        %BS = BS(ELEM(e).LocalVariate_to_GlobalVariate);
        dBS = diff(BS);
        for A = 1:nLocalNodes
            dNA = formula(dBS);
            dNA = dNA(A);
            %dNA = symfun(dNA,symvar(BS));
            %dNA = dNA(ELEM(e).LocalVariate_to_GlobalVariate);
            for B = 1:nLocalNodes
                dNB = formula(dBS);
                dNB = dNB(B);
                %dNB = symfun(dNB,symvar(BS));
                %dNB = dNB(ELEM(e).LocalVariate_to_GlobalVariate);
...
for e = 1:nELEM
    JAC = ELEM(e).Jacobian_Global_to_LocalVariate;
%     BS = C{e}*ELEM(e).LBasisFuns;
%     dNdx = diff(C{e}*ELEM(e).LBasisFuns) * JAC;
